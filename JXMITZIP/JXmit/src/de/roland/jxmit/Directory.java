...
public class Directory {
	...
	public String getDirText(String member) {
		...
		switch (dirtype) {
		case LOAD:
			return String.format("%-8s %-8s %-6s %-6s %-6s %-3s %-3s %-2s %-4s %-4s",
					e.get(0), e.get(1), e.get(2), e.get(3), e.get(4), e.get(5),
					e.get(6), e.get(7), e.get(8), e.get(9));
		case TEXT:
			return String.format(
					"%8s %8s %6s %7s %10s %16s %-8s %06X",
					e.get(0), e.get(1), e.get(2), e.get(3), e.get(4), e.get(5),
					e.get(6), ttrList.get(member));

		}
		return null;
	}

	public DirTableModel getTableModel() {
		this.tm = new DirTableModel(getHeader(), 0);
		for (int i = 0; i < textlist.size(); i++) {
			tm.addRow(textlist.get(i));
		}
		return this.tm;
	}
	...
	class DirTableModel extends DefaultTableModel {
		...
		public DirTableModel(String[] header, int i) {
			super(header, i);
		}
